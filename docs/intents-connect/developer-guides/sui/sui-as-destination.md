...
The action does not run from your own wallet. It runs from a dedicated **intermediary account** that your wallet controls through the service. On Sui this is an ed25519 account that is **deterministically derived from your origin wallet** — it is stable for a given origin wallet, but you cannot compute it yourself, so you fetch it from the API.
