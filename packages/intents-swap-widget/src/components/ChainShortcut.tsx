...
const Shortcut = ({ icon, label, isSelected, onClick }: Props) => {
  return (
    <li
      onClick={onClick}
      className={cn(
        'w-[36px] h-[36px] p-sw-md rounded-sw-md bg-sw-gray-800 hover:bg-sw-gray-700 cursor-pointer items-center justify-center flex transition-colors',
...
const AllChainsShortcut = ({
  ...
  return (
    <div
      onClick={onClick}
      className={cn(
        'px-sw-lg gap-sw-sm flex h-[36px] cursor-pointer items-center rounded-sw-md bg-sw-gray-800 hover:bg-sw-gray-700 text-sw-label-md text-sw-gray-50',
        { 'bg-sw-gray-50': isSelected },
      )}>
      <AllNetworksIcon />
