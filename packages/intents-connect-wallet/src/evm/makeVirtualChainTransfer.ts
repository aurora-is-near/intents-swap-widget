...
// `intents.near:<json msg>` where the JSON has the Intents receiver_id, so the
// asset lands inside NEAR Intents after the bridge call.
...
export const makeVirtualChainTransfer = async (
  ...
  const data = encodeFunctionData({
    ...
    args: [payload, resolveTransferAmount(args)],
  });
