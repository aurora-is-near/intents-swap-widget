...
describe('createExecutionRunner — recovery after a rejected signature', () => {
  ...
  it('resume() rebuilds and sends the deposit transfer from the execution alone', async () => {
    ...
    //
    // `depositViaWallet: true` is required for a DEPOSIT_PENDING resume: the
    // status cannot distinguish "the transfer never ran" from "it was
    // broadcast and not yet observed", so only the caller can authorise
    // sending. See the companion test below for the unauthorised default.
    ...
    const result = await createExecutionRunner({
      ...
      logger: noopLogger,
      pollIntervalMs: 0,
    }).resume('exec-1', { depositViaWallet: true });
  ...
  it('never re-sends a rebuildable DEPOSIT_PENDING deposit unprompted', async () => {
    // The double-spend window: run() broadcast the transfer, the page
    // reloaded before the watcher observed it, and the status is still
    // DEPOSIT_PENDING. The plan IS rebuildable here — everything the previous
    // test needs is present — so only the missing authorisation stops a
    // second wallet prompt for funds that are already moving.
    const signed = (status: ExecutionStatus): Execution => ({
      ...execution(status),
      quote: {
        ...execution().quote,
    ...
    expect(result.status).toBe('SUCCESS');
    expect(wallet.makeTransfer).not.toHaveBeenCalled();
    // The address is still surfaced, so a user who genuinely sent nothing can
    // fund it — and the settle loop below observed the deposit either way.
    expect(events.some((e) => e.type === 'deposit-address')).toBe(true);
  });

  it('does not re-prompt a broadcast deposit once the quote EXPIRED', async () => {
    // EXPIRED is not a special case of "no deposit yet": the deadline simply
    // passed before the watcher observed one, which is exactly the state a
    // late deposit revives. Keying the guard on the status list instead of on
    // the broadcast itself would re-prompt here.
    const statuses: ExecutionStatus[] = ['EXPIRED'];
    const api = makeApi({
      listExecutions: vi.fn(async (_w: string, query?: { id?: string }) => {
        if (!query?.id) {
          return [];
    ...
    await runner.run(basePlan).catch(() => undefined);

    expect(wallet.makeTransfer).toHaveBeenCalledTimes(1);

    await runner.resume('exec-1').catch(() => undefined);

    expect(wallet.makeTransfer).toHaveBeenCalledTimes(1);
  });

  it('does not re-prompt a deposit this runner already broadcast', async () => {
    // Same-session variant: no reload, so the runner remembers it sent the
    // transfer. `state` survives the machine reset resume() performs, which
    // context.depositTxHash does not.
    const statuses: ExecutionStatus[] = ['DEPOSIT_PENDING'];
    const api = makeApi({
      listExecutions: vi.fn(async (_w: string, query?: { id?: string }) => {
        if (!query?.id) {
          return [];
    ...
    // Still one: the resume saw the deposit it had already broadcast.
    expect(wallet.makeTransfer).toHaveBeenCalledTimes(1);
  });
  ...
  it('records no failure once the auto-cancel actually succeeded', async () => {
    // The rejection is still what run() rethrows, but cancel() has already
    // deleted the execution and cleaned the context — recording the throw
    // would put a populated `error` on a `cancelled` machine, so a UI keyed
    // off it renders a completed cancellation as a failure.
    const wallet = makeWallet();
    const baseProviders = wallet.getProviders();
    let signCalls = 0;
    ...
    wallet.getProviders = () => ({
      evm: {
        request: async (args: { method: string }) => {
          // The intent signature is rejected; the delete signature the
          // auto-cancel prompts right after is approved.
          if (args.method === 'personal_sign') {
            signCalls += 1;
  ...
  it('names the blocking execution when it refuses a re-entrant run()', async () => {
    // The parked DepositTransferError case: the machine holds exec-1 in a
    // non-terminal phase, so a second run() is refused — and the guard must
    // carry the id, or deriveExecutionRecovery cannot offer resume-or-cancel
    // and the UI dead-ends on a recoverable state.
    const wallet = makeWallet();
    ...
    const rejection = await runner.run(basePlan).catch((error) => error);

    expect(rejection).toMatchObject({
      code: 'EXECUTION_IN_FLIGHT',
      meta: { executionId: 'exec-1' },
    });
    expect(deriveExecutionRecovery(rejection)).toEqual({
...
describe('createExecutionRunner — review regressions', () => {
  it('resume() never re-prompts the deposit once funds were observed', async () => {
    // Deposit detected BEFORE the signature: the status is DEPOSIT_PROCESSING
    // while the payload is still unsigned. Resuming must sign — and must NOT
    // send a second transfer for funds the watcher already observed.
    const queue = [execution('DEPOSIT_PROCESSING'), execution('SUCCESS')];
    const api = makeApi({
      listExecutions: vi.fn(async () => [
        queue.shift() ?? execution('SUCCESS'),
      ]),
    ...
    expect(api.submitSignature).toHaveBeenCalled();
    expect(wallet.makeTransfer).not.toHaveBeenCalled();
  });

  it('holds the QR deposit screen through DEPOSIT_PENDING', async () => {
    // DEPOSIT_PENDING means signed and still waiting on funds — the first
    // poll must not flip awaiting-deposit to settling while the user is
    // still moving funds through an exchange.
    const { events, onEvent } = collect();
    const api = makeApi({
      statusSequence: ['DEPOSIT_PENDING', 'DEPOSIT_PROCESSING', 'SUCCESS'],
    });
    ...
    const settlingIndex = events.findIndex(
      (e) => e.type === 'phase' && e.phase === 'settling',
    );

    const pendingIndex = events.findIndex(
      (e) => e.type === 'status' && e.status === 'DEPOSIT_PENDING',
    );

    const processingIndex = events.findIndex(
      (e) => e.type === 'status' && e.status === 'DEPOSIT_PROCESSING',
    );

    expect(pendingIndex).toBeGreaterThan(-1);
    expect(settlingIndex).toBeGreaterThan(processingIndex);
  });

  it('feeds widget plugins the amount alias they read', async () => {
    const plugin = {
      makeTransfer: vi.fn().mockResolvedValue({ hash: '0xplugin' }),
    };

    const wallet = { ...makeWallet(), makeTransfer: undefined };
    ...
    expect(plugin.makeTransfer).toHaveBeenCalledWith(
      expect.objectContaining({ amount: '1270000', amountAtomic: '1270000' }),
      undefined,
    );
  });

  it('cancel() refuses to act after dispose()', async () => {
    const api = makeApi();
    const runner = createExecutionRunner({
      api,
      wallet: makeWallet(),
    ...
    await expect(runner.cancel('exec-1')).rejects.toThrow(/disposed/);
    expect(api.deleteExecution).not.toHaveBeenCalled();
  });

  it('a lost preflight race still carries the blocking execution id', async () => {
    // The preflight sees nothing, the create loses the race and answers 409;
    // the re-list recovers the winner's id so a UI can still offer
    // resume-or-cancel instead of a dead-end failure.
    const statusQueries = [[], [execution('CREATED')]];
    const api = makeApi({
      createExecution: vi
        .fn()
        .mockRejectedValue(
          new IntentsConnectApiError('execution already in flight', 409),
        ),
      listExecutions: vi.fn(async (_wallet: string, query?: { id?: string }) =>
        query?.id ? [execution('SUCCESS')] : (statusQueries.shift() ?? []),
      ),
    });
  ...
  it('a deliberate cancel is never reported as an error', async () => {
    const { events, onEvent } = collect();
    const api = makeApi({ statusSequence: ['DEPOSIT_PENDING'] });
  ...
  it('dispose() interrupts the polling sleep instead of waiting it out', async () => {
    const { events, onEvent } = collect();
    const api = makeApi({ statusSequence: ['DEPOSIT_PENDING'] });
    const runner = createExecutionRunner({
      api,
      wallet: makeWallet(),
      logger: noopLogger,
      // Long enough that only an interrupted sleep lets this test finish.
    ...
    // Let the flow reach the settle loop's first sleep.
    await new Promise<void>((resolve) => {
    ...
    // A disposal is not a failure: the orphaned run must stop emitting, not
    // announce a spurious error through the shared onEvent.
    expect(events.filter((e) => e.type === 'error')).toHaveLength(0);
    expect(
      events.filter((e) => e.type === 'phase' && e.phase === 'failed'),
    ).toHaveLength(0);
  });
});

describe('createExecutionRunner — second-round regressions', () => {
  /** A quote deadline safely in the future, so the deposit hold stays open. */
  const futurePlan = {
    ...basePlan,
    quote: {
      ...basePlan.quote,
  ...
  it('refuses retryDeposit() while a flow is still driving the execution', async () => {
    // QR path: run() holds in awaiting-deposit while the watcher waits on
    // funds. A retryDeposit() here would start a second settle loop — and on
    // the wallet path a second transfer prompt for the same funds.
    let current: ExecutionStatus = 'DEPOSIT_PENDING';
    const api = makeApi({
      listExecutions: vi.fn(async (_w: string, query?: { id?: string }) =>
        query?.id ? [execution(current)] : [],
      ),
    });
    ...
    const running = runner.run({ ...futurePlan, depositViaWallet: false });

    await new Promise<void>((resolve) => {
      setTimeout(resolve, 20);
    });

    await expect(runner.retryDeposit()).rejects.toMatchObject({
      code: 'EXECUTION_IN_FLIGHT',
    });
  ...
  it('cancel() of a foreign execution leaves the live one untouched', async () => {
    let current: ExecutionStatus = 'DEPOSIT_PENDING';
    const api = makeApi({
      listExecutions: vi.fn(async (_w: string, query?: { id?: string }) =>
        query?.id ? [execution(current)] : [],
    ...
    // Cleaning up a stale execution from an earlier session must delete it —
    // and must NOT wipe the live execution's context or force its phase.
    await runner.cancel('stale-exec-from-last-week');

    expect(api.deleteExecution).toHaveBeenCalledWith(
      ADDRESS,
      'stale-exec-from-last-week',
      expect.anything(),
    );
    expect(runner.getPhase()).not.toBe('cancelled');
    expect(runner.getStore().context.executionId).toBe('exec-1');
  ...
  it('re-confirms a stale preflight before failing EXECUTION_IN_FLIGHT', async () => {
    // The concurrent preflight snapshot is as old as planning took; an
    // execution it saw open may have completed since. The fresh re-list must
    // let the run proceed instead of failing it.
    const statusQueries = [[execution('CREATED')], []];
    const api = makeApi({
      listExecutions: vi.fn(async (_w: string, query?: { id?: string }) =>
        query?.id ? [execution('SUCCESS')] : (statusQueries.shift() ?? []),
...
describe('createExecutionRunner — third-round regressions', () => {
  it('fills an EVM origin chain id from the registry when the plan omits it', async () => {
    // basePlan carries no `originChainId` — the shape the README documents.
    // Without the registry fallback the transfer receives `evmChainId: null`,
    // which the EVM plugin refuses AFTER the signature is already submitted,
    // stranding the execution with retryDeposit() replaying it forever.
    const wallet = makeWallet();
    ...
    expect(wallet.makeTransfer).toHaveBeenCalledTimes(1);
    expect(vi.mocked(wallet.makeTransfer!).mock.calls[0]![0]).toMatchObject({
      chain: 'base',
      evmChainId: 8453,
    });
  });

  it('arms the network-mismatch guard for an EVM plan with no chain id', async () => {
    // Same fallback, other consequence: a wrong-network wallet now fails
    // during planning instead of at the deposit, so nothing is created.
    const api = makeApi();
    ...
    await expect(
      ...
      /wallet is on chain 1 but the origin requires chain 8453/,
    );

    expect(api.createExecution).not.toHaveBeenCalled();
  });

  it('leaves a non-EVM origin chain id alone', async () => {
    const wallet = makeWallet();
  ...
  it('resume({ depositViaWallet: false }) never spends a retained plan', async () => {
    // An out-operation run (/submit answers SIGNING) ends terminally WITHOUT
    // depositing, so the plan stays retained with nothing broadcast — the state
    // the opt-out has to survive. The retained plan carries the original run's
    // `depositViaWallet: true`, and the opt-out is unconditional, so it must
    // gate that plan too rather than only the rebuilt one.
    const submitStatuses: SubmitStatus[] = [
      'SIGNING',
      'SIGNED_PENDING_DEPOSIT',
    ];

    const idQueries: ExecutionStatus[] = ['CREATED', 'CREATED', 'SUCCESS'];

    const api = makeApi({
      submitSignature: vi.fn(async () => ({
        status: submitStatuses.shift() ?? 'SIGNED_PENDING_DEPOSIT',
      })),
      listExecutions: vi.fn(async (_w: string, query?: { id?: string }) => {
        if (!query?.id) {
          return [];
        ...
        const next = idQueries.length > 1 ? idQueries.shift() : idQueries[0];
    ...
    // Times out at settle, leaving the machine terminal and the plan retained.
    await expect(runner.run(basePlan)).rejects.toThrow();
    expect(wallet.makeTransfer).not.toHaveBeenCalled();

    await expect(
      runner.resume('exec-1', { depositViaWallet: false }),
    ).resolves.toMatchObject({ status: 'SUCCESS' });

    // The address was surfaced instead of the retained plan being spent.
    expect(wallet.makeTransfer).not.toHaveBeenCalled();
  });

  it('does not record a deposit broadcast after the execution was cancelled', async () => {
    // cancel() lands while the transfer prompt sits open. The tx is already
    // broadcast, so the id must be remembered — but nothing may describe an
    // execution that no longer exists server-side.
    const api = makeApi({ statusSequence: ['CREATED'] });
    const wallet = makeWallet();
    const { events, onEvent } = collect();
    ...
    vi.mocked(wallet.makeTransfer!).mockImplementationOnce(async () => {
      await runner.cancel('exec-1');

      return { hash: '0xdeposit' };
    });

    await expect(runner.run(basePlan)).rejects.toThrow(/cancelled/);
