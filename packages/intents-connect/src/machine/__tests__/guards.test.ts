...
describe('stepsRequiredForRealCreate', () => {
  it('allows a non-empty steps array', () => {
    expect(() => guards.stepsRequiredForRealCreate([step()])).not.toThrow();
  });

  it('rejects an empty steps array on a real create', () => {
    expectGuard('STEPS_REQUIRED', () => guards.stepsRequiredForRealCreate([]));
  });
});
...
describe('memoPresentForStellar', () => {
  ...
  it('fails CLOSED when the origin chain could not be determined', () => {
    // "Cannot tell" must not read as "not memo mode": that is exactly how a
    // memo-mode deposit reaches the shared address without its memo, which
    // loses the funds rather than failing them.
    expectGuard('MEMO_REQUIRED', () =>
      guards.memoPresentForStellar(undefined, null),
    );

    expectGuard('MEMO_REQUIRED', () =>
      guards.memoPresentForStellar(undefined, null, 'ton_connect'),
    );
  });

  it('allows an unknown origin when the memo is already there', () => {
    // The guard only ever catches a MISSING memo — refusing a deposit that
    // carries one would block it for no reason, whatever chain it is on.
    expect(() => guards.memoPresentForStellar(undefined, 'memo')).not.toThrow();

    expect(() =>
      guards.memoPresentForStellar(undefined, 'memo', 'ton_connect'),
    ).not.toThrow();
  });

  it('allows an unknown origin for a standard that cannot be memo-mode', () => {
    // erc191/raw_ed25519/nep413 provably never originate on a memo-mode
    // chain, so an unresolved origin there is safe to pass.
    ['erc191', 'raw_ed25519', 'nep413'].forEach((standard) => {
      expect(() =>
        guards.memoPresentForStellar(undefined, null, standard),
      ).not.toThrow();
    });
  });
});
