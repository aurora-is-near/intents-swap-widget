...
describe('StrictMode', () => {
  it('survives the dev double-mount without bricking the runner', async () => {
    // React 18 StrictMode runs mount -> cleanup -> remount while useMemo keeps
    // returning the same runner; an eager dispose in the cleanup would leave
    // the hook permanently rejecting with RunnerDisposedError.
    let start: (() => Promise<unknown>) | undefined;
    ...
    // Let the deferred-dispose timers from the simulated unmount settle.
    await act(async () => {
...
describe('IntentsConnectProvider', () => {
  it('forwards autoCancelOnSignatureRejection instead of dropping it', () => {
    // The provider rebuilds the context object field by field; a runner
    // option missing from that rebuild is silently swallowed and the runner
    // falls back to its default with no type error.
    let seen: boolean | undefined;

    const Probe = () => {
      seen = useIntentsConnect().autoCancelOnSignatureRejection;
